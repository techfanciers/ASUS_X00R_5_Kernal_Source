		...
		const char *psy_name,  const char  *batt_type)
{
	...
	pr_err("wind %s:A306 batt_id_kohm=%d\n", __func__,batt_id_kohm);
	if(batt_id_kohm >= 45 && batt_id_kohm <= 55){
	sprintf(wind_device_info.battery_data.BAT_Model_Name, "%s", "C11P1709");
	...
	sprintf(wind_device_info.battery_data.Image_Version, "%s", "15.00.1712.0"); 
		}
	#else 
