	...
	struct mdss_dsi_ctrl_pdata *ctrl)
{
    ...
    // wangjun@wind-mobi.com 20180411 begin
    {
        int i,j;
        for (j = 0; j < ctrl->groups; ++j) {
    		for (i = 0; i < status_len; ++i) {			
                // wangjun@wind-mobi.com 20180411 for esd log begin
                {
                    printk("lcd esd status_value = 0x%x %s %d\n", ctrl->status_value[i],  __func__, __LINE__);
                }
                // wangjun@wind-mobi.com 20180411 for esd log end
    		}
    	}
    }
    // wangjun@wind-mobi.com 20180411 end
