...
void  rf_temp_check_handler(unsigned long data)
{
	...
	printk("yutao : enter rf_temp_check_handler,cur_rf_check_state =%d\n",cur_rf_check_state);
	   if (next_rf_check_state) {
        ...
        // wangjun@wind-mobi.com 20180402 begin         
        if(!down_interruptible(&(rf_status_dev->sem)))
		{
			rf_status_dev->rf_status_val=0;
    	    up(&(rf_status_dev->sem));
            printk("[wjwind] out plug!");
		}    	        
        // wangjun@wind-mobi.com 20180402 end 
        ...
        if(!down_interruptible(&(rf_status_dev->sem)))
		{
			rf_status_dev->rf_status_val=1;
    	    up(&(rf_status_dev->sem));
            printk("[wjwind] in plug!");
		} 
        // wangjun@wind-mobi.com 20180402 end 
...
{   
	...
	{ 
		...
		if (!rf_temp_check_irq)
           ...
           if(gpio_get_value(GPIO_RF_STATUS_CHECK_PIN))
			{
				ret=request_irq(rf_temp_check_irq, rf_temp_check_eint_handler, IRQF_TRIGGER_FALLING, "rf_tem_det-eint", NULL);
			}
		   else
		   {
			    ret=request_irq(rf_temp_check_irq, rf_temp_check_eint_handler, IRQF_TRIGGER_RISING, "rf_tem_det-eint", NULL); 
		   }
