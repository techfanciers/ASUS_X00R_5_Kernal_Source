...
// wangbing@wind-mobi.com 20180423 begin >>> [1/7] add tp probe flag
int tp_is_probe = 0;
// wangbing@wind-mobi.com 20180423 end   <<< [1/7] add tp probe flag
...
#include "YJ_EK55M.21434A_YD_A305_C01_2018-01-11_1018.i"
// wangjun@wimd-mobi.com 20180301 end
};


// qcom,mdss-dsi-panel-name = "st7703 720p video mode dsi panel";
unsigned char HSD_ST7730_HX8527[]=
{
// wangjun@wimd-mobi.com 20180301 begin
#include "EK55M.21434A_YJ_YD_A305_CID3312_5A_C0C_05232018.i"
// wangjun@wimd-mobi.com 20180301 end
};


// qcom,mdss-dsi-panel-name = "dsi_boe55_st7703_720p_video";
unsigned char BOE_ST7730_HX8527[]=
{
// zhaopengfei@wimd-mobi.com 20180403 begin	>>TP2_FW_UPDATE
#include "LTFBF0551536_LC_ASUS_A306_CID3702_5A_C34_06052018.i"
// zhaopengfei@wimd-mobi.com 20180403 end	>>TP2_FW_UPDATE
};
...
//zhangkai@wind-mobi.com 20180227 start
#ifdef HX_USB_DETECT_GLOBAL
extern bool usb_present;
#endif
//zhangkai@wind-mobi.com 20180227 end
...
int himax_input_register(struct himax_ts_data *ts)
{
    ...
    set_bit(KEY_CUST_15, ts->input_dev->keybit);//hongfan@wind-mobi.com add for double tap at 20180206
#endif
...
static int i_update_FW(void)
{
    ...
    if(!strcmp("st7703 720p video mode dsi panel",caPanelName)){
        ...
        ImageBuffer = HSD_ST7730_HX8527;
        fullFileLength = sizeof(HSD_ST7730_HX8527);
        printk("[wjwind] current Update = %s\n",caPanelName);
    }
    else if(!strcmp("dsi_boe55_st7703_720p_video",caPanelName))
    {
        ImageBuffer = BOE_ST7730_HX8527;
        fullFileLength = sizeof(BOE_ST7730_HX8527);
        printk("[wjwind] current Update = %s\n",caPanelName);
    }
    else
    {
...
// wangbing@wind-mobi.com 20180319 begin >>> [1/5] realize the touch panel gesture feature
    ...
    if(0x80 == ret_event) {
        ...
        {
                paul("the gesture not enable or not support");
                return;
        }
    }
// wangbing@wind-mobi.com 20180319 end   <<< [1/5] realize the touch panel gesture feature
    ...
    {
    case EV_GESTURE_PWR:
        /* #define KEY_CUST_01 KEY_GESTURE_DOUBLE_CLICK */
        KEY_EVENT = KEY_GESTURE_DOUBLE_CLICK;               // zhaopengfei@wind-mobi.com modify 20180329
        paul("gesture: double click");
        break;
    case EV_GESTURE_01:
        /* #define KEY_CUST_01 KEY_GESTURE_UP */
    ...
    case EV_GESTURE_04:
        ...
        break;
    case EV_GESTURE_05:
        /* #define KEY_CUST_05 KEY_GESTURE_C */
        KEY_EVENT = KEY_CUST_05;
        paul("gesture: c");
        break;
    case EV_GESTURE_06:
        /* #define KEY_CUST_06 KEY_GESTURE_Z */
    ...
    case EV_GESTURE_08:
        ...
        break;
    case EV_GESTURE_09:
        /* #define KEY_CUST_09 KEY_GESTURE_S */
        KEY_EVENT = KEY_CUST_09;
        paul("gesture: s");
        break;
    case EV_GESTURE_10:
        /* #define KEY_CUST_10 KEY_GESTURE_V */
        KEY_EVENT = KEY_CUST_10;
        paul("gesture: v");
        break;
    case EV_GESTURE_11:
        /* #define KEY_CUST_11 KEY_GESTURE_W */
        KEY_EVENT = KEY_CUST_11;
        paul("gesture: w");
        break;
    case EV_GESTURE_12:
        /* #define KEY_CUST_12 KEY_GESTURE_E */
...
void himax_cable_detect_func(bool force_renew)
{
    ...
    connect_status = usb_present;//upmu_is_chr_det();    //zhangkai@wind-mobi.com 20180227
    ts = private_ts;
    ...
    if (hx_point_num != 0 )
    {
        ...
        for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++)
        {
            ...
            if(x >= 0 && x <= ts->pdata->abs_x_max && y >= 0 && y <= ts->pdata->abs_y_max)
            {
                 ...
                 * 2. using "dmesg -w | grep -E 'P[01234]'" to view the log
                 */
                // printk("P%d(%d,%d)\n", loop_i, x, y);
...
// wangbing@wind-mobi.com 20180423 begin >>> [2/7] add tp probe flag
    paul("enter, tp_is_probe = %d", tp_is_probe);
    if(tp_is_probe)
    {
        paul("tp has probe");
        err = -ENODEV;
...
// wangbing@wind-mobi.com 20180423 end   <<< [2/7] add tp probe flag
    ...
    printk("[wjwind] current name = %s\n",caPanelName);// wangjun@wind-mobi.com 20180314 add
    printk("[wjwind] IC fw =0x%x conf = 0x%x Firmware: fw = 0x%x, conf = 0x%x\n",ic_data->vendor_fw_ver,ic_data->vendor_config_ver,g_i_FW_VER,g_i_CFG_VER);// wangjun@wind-mobi.com 20180314 add
    if (auto_update_flag)
    {
    ...
    g_chip_monitor_data->HX_CHIP_MONITOR_EN = 1;
    printk("wind-hf init himax_chip_monitor_function \n");
...
#ifdef HX_SMART_WAKEUP
    ts->SMWP_enable=0;//wangjun@wind-mobi 20180323 change to 0
...
// wangbing@wind-mobi.com 20180423 begin >>> [3/7] add tp probe flag
    tp_is_probe = 1;
    paul("done, tp_is_probe = %d", tp_is_probe);
// wangbing@wind-mobi.com 20180423 end   <<< [3/7] add tp probe flag
...
// wangbing@wind-mobi.com 20180504 begin >>> [1/3] remove the touch point when tp suspend
...
// wangbing@wind-mobi.com 20180504 end   <<< [1/3] remove the touch point when tp suspend
...
// wangbing@wind-mobi.com 20180504 begin >>> [2/3] remove the touch point when tp suspend
	himax_report_all_leave(ts);
// wangbing@wind-mobi.com 20180504 end   <<< [2/3] remove the touch point when tp suspend
        return 0;
    }
#endif
...
// wangbing@wind-mobi.com 20180504 begin >>> [3/3] remove the touch point when tp suspend
    himax_report_all_leave(ts);
// wangbing@wind-mobi.com 20180504 end   <<< [3/3] remove the touch point when tp suspend
